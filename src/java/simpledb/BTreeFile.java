...
public class BTreeFile implements DbFile {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		if (page == null)
			throw new DbException("Null page");
...
//        get the parent of the new page
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), key_field);
		
		BTreeEntry new_entry = new BTreeEntry(key_field, page.getId(), new_page.getId());
		parent.insertEntry(new_entry);
		updateParentPointers(tid, dirtypages, parent);
		
		new_page.setParentId(parent.getId());
		
		if (field.compare(Op.GREATER_THAN_OR_EQ, key_field))
			return new_page;
		return page;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		if (page.getNumEntries() > new_page.getNumEntries())
			key_entry = it.next();
		else
			key_entry = current_entry;
		
		Field key_field = key_entry.getKey();
		page.deleteKeyAndRightChild(key_entry);
		key_entry.setLeftChild(page.getId());
		key_entry.setRightChild(new_page.getId());
		parent.insertEntry(key_entry);
