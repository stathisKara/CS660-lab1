...
public class BTreeFile implements DbFile {
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        BTreeHeaderPage header = (BTreeHeaderPage) getPage(tid, dirtypages, root.getHeaderId(), Permissions.READ_ONLY);
//        header.numSlots
        BTreeLeafPage insertion_page = findLeafPage(tid, dirtypages, page.getId(), Permissions.READ_WRITE, field);
        if (insertion_page == null) {
            return null;
        }
        page.getPageData();
        BTreePageId newId = new BTreePageId(tableid, 3, BTreePageId.LEAF);
        BTreeLeafPage new_page = new BTreeLeafPage(newId, page.getPageData(), keyField);

//        Fix pointers
        new_page.setRightSiblingId(page.getRightSiblingId());
        new_page.setLeftSiblingId(page.getId());
        new_page.setParentId(page.getParentId());
        page.setRightSiblingId(new_page.getId());
        ((BTreeLeafPage) getPage(tid, dirtypages, new_page.getRightSiblingId(), Permissions.READ_ONLY)).setLeftSiblingId(new_page.getId());
