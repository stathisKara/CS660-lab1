...
//		System.out.println("----------------------");
		...
		int freeSlotPos = -1;
		int i = 0;
		while (freeSlotPos == -1 && i < this.header.length) {
			for (int bit = 0; bit < 8; bit++) {
				if (!isSlotUsed((i * 8) + bit)) {
					freeSlotPos = (i * 8) + bit;
					break;
				}
			}
		...
		if (freeSlotPos == -1)
			throw new DbException("No space in page.");
		
		t.setRecordId(new RecordId(this.pid, freeSlotPos));
		tuples[freeSlotPos] = t;
		markSlotUsed(freeSlotPos, true);
